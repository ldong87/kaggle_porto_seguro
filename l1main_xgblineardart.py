...
def forloop(j):
    ...
    def one_pass(min_child_weight,colsample_bytree,max_depth,subsample,gamma,alpha,scale_pos_weight):
        ...
        pred_tst[:,j] = 0
        print 'jfold: ', j
        pred_trn_tmp, pred_tst_tmp, best_iter = m1xgblinear.model_pred(trn_tmp_x,trn_tmp_y,val_tmp_x,val_tmp_y,tst_x,
        ...
        for i in xrange(nfold):
            if i==j: 
                ...
                print 'ifold: ', i
                pred_trn_tmp, pred_tst_tmp, best_iter = m1xgblinear.model_pred(trn_tmp_x_,trn_tmp_y_,val_tmp_x_,val_tmp_y_,tst_x,
                                                                  min_child_weight,colsample_bytree,max_depth,subsample,gamma,alpha,
                                                                  scale_pos_weight)
...
#             modelBO.res['max']['max_params']['scale_pos_weight'])
    ...
    one_pass(14.8502,0.5660,4,0.9776,5.7430,3.4937,3.6465)
...
#
#save_pred(train.id,test.id,nfold,pred_trn,pred_tst,'m1_xgblinear')
...
def forloop(j):
    ...
    def one_pass(min_child_weight,colsample_bytree,max_depth,subsample,gamma,alpha,scale_pos_weight,rate_drop,skip_drop):
        ...
        pred_tst[:,j] = 0
        print 'jfold: ', j
        pred_trn_tmp, pred_tst_tmp, best_iter = m1xgbdart.model_pred(trn_tmp_x,trn_tmp_y,val_tmp_x,val_tmp_y,tst_x,
        ...
        for i in xrange(nfold):
            if i==j: 
                ...
                val_tmp_y_ = trn_y.iloc[cv_ind[ flag_valid[:,i]]]
                print 'ifold: ', i
                pred_trn_tmp, pred_tst_tmp, best_iter = m1xgbdart.model_pred(trn_tmp_x_,trn_tmp_y_,val_tmp_x_,val_tmp_y_,tst_x,
                                                                  min_child_weight,colsample_bytree,max_depth,subsample,gamma,alpha,scale_pos_weight,rate_drop,skip_drop)
                print 'double check gini of ifold=', i,' pred = ', eval_gini(val_tmp_y_, pred_trn_tmp)
                pred_trn[cv_ind[flag_valid[:,i]],j] = pred_trn_tmp
                pred_tst[:,j] = pred_tst[:,j] + pred_tst_tmp
...
#             modelBO.res['max']['max_params']['skip_drop'])
    ...
    one_pass(18.0241,0.8930,9,0.846,12.3867,18.2430,1.8925,0.2056,0.9688)    
    print "\nGini for full training set ", j, ':', eval_gini(trn_y, pred_trn[:,j])
...
save_pred(train.id,test.id,nfold,pred_trn,pred_tst,'m1_xgbdart')
