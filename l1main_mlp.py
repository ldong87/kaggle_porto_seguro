...
def forloop(j):
    ...
    def one_pass(numl1,numl2,numl3):
        global pred_trn, pred_tst
        pred_tst[:,j] = 0
        print 'jfold: ', j
        pred_trn_tmp, pred_tst_tmp, best_iter = m1mlp.model_pred(trn_tmp_x,trn_tmp_y,val_tmp_x,val_tmp_y,tst_x,
                                                      numl1,numl2,numl3)
        ...
        for i in xrange(nfold):
            if i==j: 
                ...
                print 'ifold: ', i
                pred_trn_tmp, pred_tst_tmp, best_iter = m1mlp.model_pred(trn_tmp_x_,trn_tmp_y_,val_tmp_x_,val_tmp_y_,tst_x,
                                                              numl1,numl2,numl3)
...
#    modelBO = BO(one_pass, {    'numl1': (2,20),
#                                'numl2': (2,20),
#                                'numl3': (2,20)
#                                }, random_state=1987)
#    modelBO.explore({'numl1': [10],
#                     'numl2': [10],
#                     'numl3': [10]})
#    modelBO.maximize(init_points=10, n_iter=20, acq='rnd')
#    
#    one_pass(modelBO.res['max']['max_params']['numl1'],
#             modelBO.res['max']['max_params']['numl2'],
#             modelBO.res['max']['max_params']['numl3']
#             )
    one_pass(10,10,10)
...
result_combo = [forloop(i) for i in xrange(nfold)]
pred_trn = np.transpose(np.array([result_combo[i][0] for i in xrange(nfold)]))
pred_tst = np.transpose(np.array([result_combo[i][1] for i in xrange(nfold)]))
t1 = timer()
print 'Timer mlp:', t1-t0

print "\nGini for full training set avg:" , eval_gini(trn_y, np.mean(pred_trn,axis=1))

save_pred(train.id,test.id,nfold,pred_trn,pred_tst,'m1_mlp')
