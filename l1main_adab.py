...
def forloop(j):
    ...
    def one_pass(lr):
        ...
        pred_tst[:,j] = 0
        print 'jfold: ', j
        pred_trn_tmp, pred_tst_tmp, best_iter = m1adab.model_pred(trn_tmp_x,trn_tmp_y,val_tmp_x,val_tmp_y,tst_x,
                                                          lr)
        ...
        for i in xrange(nfold):
            if i==j: 
                ...
                print 'ifold: ', i
                pred_trn_tmp, pred_tst_tmp, best_iter = m1adab.model_pred(trn_tmp_x_,trn_tmp_y_,val_tmp_x_,val_tmp_y_,tst_x,
                                                                  lr)
...
t1 = timer()
print 'Timer adab:', t1-t0

print "\nGini for full training set avg:" , eval_gini(trn_y, np.mean(pred_trn,axis=1))

save_pred(train.id,test.id,nfold,pred_trn,pred_tst,'m1_adab')
